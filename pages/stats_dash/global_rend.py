...
def show_page():
    ...
    select_variable=st.selectbox(label="Elija la métrica a visualizar",options=numeric_cols_list)
    
    col1,col2=st.columns([1,1])
    with col1:
        lower_better = st.checkbox("📉 De menor a mayor",help="Marque esta opción si un valor menor implica mejor rendimiento.")
    with col2:
        per_90 = st.checkbox("🎯 Mostrar métrica por 90'",help="Marque esta opción si un valor menor implica mejor rendimiento.")
    values = st.slider("Seleccione el número de jugadores que desea visualizar", 0,50, 10)
    
    if per_90:
        df=df[df["Minutos jugados"]>90]
        if "90" not in select_variable:
            df[select_variable] = np.where(
                df["Minutos jugados"] > 0,
                90 * df[select_variable] / df["Minutos jugados"],
                0  
                )
    
    df_display = (
        df
        .dropna(subset=select_variable)  # remove rows with NaN in selected metrics
        .sort_values(by=select_variable, ascending=lower_better)
    )
    ...
    fig = px.bar(df_display, x='Jugador', y=select_variable,color=select_variable,text=select_variable,color_continuous_scale=['violet', 'yellow'])
