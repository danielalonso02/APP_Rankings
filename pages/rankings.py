...
# ── Persistencia del tab activo via query_params + JS ───────────────────────
_TAB_LABELS = ["📊 Rankings de Rendimiento", "🔍 Visualizador por Métrica", "📋 Tabla Multi-Métrica", "🧤 Porteros"]

# JS: guarda el tab activo en la URL al hacer clic, y restaura el tab al recargar
st.markdown("""
<script>
(function() {
    function attachTabListeners() {
        const buttons = window.parent.document.querySelectorAll('[data-baseweb="tab"]');
        if (!buttons.length) { setTimeout(attachTabListeners, 300); return; }
    ...
    function clickSavedTab() {
        const params = new URLSearchParams(window.parent.location.search);
        const idx = parseInt(params.get('tab') || '0');
        const buttons = window.parent.document.querySelectorAll('[data-baseweb="tab"]');
        if (!buttons.length) { setTimeout(clickSavedTab, 300); return; }
