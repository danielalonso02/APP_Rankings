...
def create_report(player_id1,player_id2,wyscout_file,parameters_file,position_number,min_minutes,color_selection="#FFFFFF",summary=0,current_league=current_l,season="2024/25"):
    ...
    folder_skillcorner = "/Users/danie/Desktop/SKILLCORNER/"
    ...
    try:
        ...
        player_score,player_letter,number_of_players_position_fis1, playerranking = 0, "-", 0, 0
        
        
    if (player_minutes1>min_minutes) and (player_minutes2>min_minutes):
        fig4,output_phys,player1_params,player2_params,parameters1_show,phys_average1=pizzaplot_phys(player_name1,player_name2,PLAYER_POSITION,folder_skillcorner,league=current_league, color="#FFFFFF")
        #print(output_path2)
    else:
        print(f"El jugador ha jugado menos de {min_minutes} minutos, selecciona otro jugador o baja el número de minutos necesarios.")
        return None
   ...
   # TEAM_LOGO = complete_teampath
    ...
    paths_graficas_2D=graficas_2D(wyscout_file,player_id1,player_id2,PLAYER_POSITION,min_minutes,folder_skillcorner,color_selection_og)
    ...
    data2 = [
        ["Jugadora", "Pie dominante", "Pasaporte","País de nacimiento"],
        [player_name1, player_foot1, player_nationality1,player_birth_country1],
        [player_name2,player_foot2, player_nationality2,player_birth_country2]
        ]

    # Crear tablas
    table1 = Table(data1, colWidths=[100,80, 50, 100, 100])
    table2 = Table(data2, colWidths=[100, 80, 60, 130])
    ...
    data_t3=[
        ...
        [" ",f"{d41}/{number_of_players_position1}",f"{d42}/{number_of_players_position1}",f"{d43}/{number_of_players_position1}",f"{d44}/{number_of_players_position_fis1}"],
        [player_name2,Paragraph_d12,Paragraph_d22,Paragraph_d32,Paragraph_d42],
        [" ",f"{d412}/{number_of_players_position2}",f"{d422}/{number_of_players_position2}",f"{d432}/{number_of_players_position2}",f"{d442}/{number_of_players_position_fis2}"]
        ]
    style = TableStyle([
    ...
    texto_aclaración="* Valoración en función de las variables ponderadas según la posición de la jugadora"
    ...
    for i in range(len(player1_params)):
        ...
        bar = create_bar_chart(
            phys_value1, # El número que se muestra
            phys_value2,
            phys_value1,
            phys_value2, # El relleno de la barra
    ...
    for letter in letters:
        if letter == f_let:
            # Letra grande y en color
            formatted_phys2 += f'<font color="{f_col}" size="48"><b>{letter}</b></font>&nbsp;  '
        else:
            # Letras normales
