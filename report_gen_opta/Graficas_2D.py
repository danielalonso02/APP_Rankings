...
def graficas_2D(filepath_excel,player_id_analizing,player_position,min_minutos,folder_skillcorner,color="#FFFFFF"):
    ...
    # 1. Preparación de datos físicos
    # Buscamos la columna de nombre (Short Name o Jugador)
    col_phys_name = "Short Name" if "Short Name" in df_phys.columns else "Jugador"
    
    # 3. MERGE ESTRATÉGICO: 
    # Usamos df_position (que ya está filtrado por posición en Wyscout) 
    # para "limpiar" el dataframe físico.
    df_phys_pos = pd.merge(
        df_position[["Jugador"]], # Solo necesitamos la lista de nombres filtrados
        df_phys, 
        left_on="Jugador", 
        right_on=col_phys_name, 
        how="inner"
    ).copy()
    
    # Mapeo de posiciones para SkillCorner (debe coincidir con tu dictionary_positions anterior)
    phys_pos_dict = {"delantera": "Center Forward", "defensa": "Central Defender",
                     "ofmid": "Midfield", "defmid": "Midfield",
                     "lateral": "Full Back", "extremo": "Wide Attacker"}
    

    # Función auxiliar para generar scatter físico (para no repetir código)
    def save_phys_scatter(df, x_col, y_col, x_label, y_label, filename):
        fig, ax = plt.subplots(figsize=(12, 12), facecolor=color)
        ax.set_facecolor(color)
        
        # Puntos de todos los jugadores de la posición
        sns.scatterplot(data=df, x=x_col, y=y_col, s=100, alpha=0.4, color='blue')
        
        # Nombres difuminados
        for _, row in df.iterrows():
            plt.text(row[x_col], row[y_col], row[col_phys_name], fontsize=10, alpha=0.2)
        ...
        # Resaltar jugador analizado
        jugador_phys = df[df[col_phys_name] == player_name]
        if not jugador_phys.empty:
            ...
            plt.text(px, py + (df[y_col].max()*0.01), player_name, fontsize=13, weight='bold', 
                     bbox=dict(facecolor='white', edgecolor='red', pad=3), zorder=6)
    ...
    # --- Gráfico Físico 1: Volumen vs Intensidad ---
    ...
    # --- Gráfico Físico 2: Velocidad vs Sprint ---
    ...
    # --- Gráfico Físico 3: Aceleración vs Deceleración (Alta Intensidad) ---
    ...
    output_paths=[output_path1,output_path2,output_path3,output_path4,output_path5,
                  ...
                  output_path11,out_phys1, out_phys2, out_phys3]
    return output_paths
