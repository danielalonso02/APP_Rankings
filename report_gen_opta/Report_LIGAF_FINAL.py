...
def create_report(player_id_analizing,wyscout_file,parameters_file,position_number,min_minutes,color_selection="#FFFFFF",summary=0,current_league=current_l,season="2024/25"):
    ...
    if pd.isna(player_foot) or str(player_foot).strip() in ["", "nan", "None", "Unknown"]:
    ...
    folder_skillcorner = "/Users/danie/Desktop/SKILLCORNER/" # Carpeta donde están tus CSV de SkillCorner
   ...
   # TEAM_LOGO = complete_teampath
    ...
    chat_comment = f"Write a paragraph highlighting the physical strengths and weaknesses of the player {player_name} in the position {dict_english[player_pos]} with a SWOT style. The parameters we are evaluating are; {parameters_show}. The player's values are: {player_params}. The average values are: {phys_average}"
    chat(chat_comment, context, action="translate", language="Spanish",chat_log=None)
