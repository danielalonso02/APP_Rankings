...
from PizzaWyscout2RFEF_comparative import pizzaplot_player
from IntentoArrays_comparative import extract_arrays_wyscout
import time
from PIL import Image as PILImage
import io
from Graficas_2D_comparative import graficas_2D
from reportlab.platypus import KeepInFrame
from Notas_tacticas import notas_tacticas
from Notas_Skillcorner import notas_fisicas
from PizzaPhysical_comparative import pizzaplot_phys
...
def create_report(player_id1,player_id2,wyscout_file1,parameters_file,position_number,min_minutes,color_selection="#FFFFFF",summary=0,league1=current_l,league2=current_l,season1="2024/25",season2="2024/25",wyscout_file2=None,pesos_favorito=None,pesos_fisicos=None):
    
    
    
    # Si no se pasa wyscout_file2, se usa el mismo que wyscout_file1 (misma liga)
    if wyscout_file2 is None:
        wyscout_file2 = wyscout_file1

    # Liga/temporada combinada para el título del informe
    if league1 == league2 and season1 == season2:
        current_league = league1
        season = season1
    else:
        current_league = f"{league1} / {league2}"
        season = f"{season1} / {season2}"
    ...
    if not os.path.exists(wyscout_file1):
        print(f"El fichero {wyscout_file1} no existe.")
        return None
    if not os.path.exists(wyscout_file2):
        print(f"El fichero {wyscout_file2} no existe.")
        return None
    ...
    check1=pd.read_excel(wyscout_file1)
    check2=pd.read_excel(wyscout_file2)
    if player_id1>len(check1):
        print("Player 1 not valid")
        return None
    if player_id2>len(check2):
        print("Player 2 not valid")
        return None
    try:
        min_of1, max_of1, player_of11,player_of12, param_of1,df_all_stats,percentiles_of11,percentiles_of12,_,player_id2_combined = extract_arrays_wyscout(
            wyscout_file1,wyscout_file2,parameters_file,
            player_id1,player_id2,
            "param_of1", min_minutes
        )
        min_of2, max_of2, player_of21,player_of22, param_of2, _ ,percentiles_of21,percentiles_of22,_,_= extract_arrays_wyscout(
            wyscout_file1,wyscout_file2,parameters_file,
            player_id1,player_id2,
            "param_of2",min_minutes
        )
        min_def, max_def, player_def1,player_def2, param_def, _,percentiles_def1,percentiles_def2,_,_ = extract_arrays_wyscout(
            wyscout_file1,wyscout_file2,parameters_file,
            player_id1,player_id2,
            "param_def",min_minutes
        )
    except Exception as e:
        print(f"[ERROR] No se pudieron extraer los datos comparativos: {e}")
        return None
    ...
    if any(x is None for x in [min_of1, max_of1, player_of11, player_of12, param_of1, df_all_stats, percentiles_of11, percentiles_of12]):
        print("[ERROR] Las jugadoras no comparten posición o no hay suficientes datos para comparar.")
        return None
    
    # Si misma liga, player_id2_combined coincide con player_id2
    if player_id2_combined is None:
        player_id2_combined = player_id2
    ...
    positions_name_dict1={"portera":"Portera","defensa":"Defensa","lateral":"Lateral",
                         "defmid":"Centrocampista <br /> defensivo","ofmid":"Centrocampista <br /> ofensivo",
                         "extremo":"Extremo","delantera":"Delantera"}
    df_general_stats=df_all_stats[["Jugador","Equipo","Equipo durante el período seleccionado","Edad","Partidos jugados","Minutos jugados","player_id",'general_position', 'general_position2',"general_position3"]].copy()
    ...
    player_age1=int(general_stats_player1["Edad"])
    ...
    def _safe_pos(val):
        """Convierte NaN, None, 'None', '' a 'None' string para que no rompa el dict lookup."""
        if val is None or (isinstance(val, float) and pd.isna(val)) or str(val).strip() in ("", "None", "nan"):
            return "None"
        return str(val).strip()

    player_games1=general_stats_player1["Partidos jugados"]
    player_minutes1=general_stats_player1["Minutos jugados"]
    player_position_original1=_safe_pos(general_stats_player1["general_position"])
    player_position11=positions_name_dict.get(_safe_pos(general_stats_player1["general_position"]), "Defensa")
    player_param1=stats_player1["general_position"]
    
    ###jugador 2
    general_stats_player2 = df_general_stats[df_general_stats["player_id"] == player_id2_combined].iloc[0]
    stats_player2 = df_all_stats[df_all_stats["player_id"] == player_id2_combined].iloc[0]
    ...
    player_age2 = int(general_stats_player2["Edad"])
    ...
    player_position_original2 = _safe_pos(general_stats_player2["general_position"])
    player_position22 = positions_name_dict.get(_safe_pos(general_stats_player2["general_position"]), "Defensa")
    player_param2 = stats_player2["general_position"]
    #print(general_stats_player1)
    
    player1_positions=[_safe_pos(general_stats_player1["general_position"]),_safe_pos(general_stats_player1["general_position2"]),_safe_pos(general_stats_player1["general_position3"])]
    player2_positions=[_safe_pos(general_stats_player2["general_position"]),_safe_pos(general_stats_player2["general_position2"]),_safe_pos(general_stats_player2["general_position3"])]

    common=set(player1_positions) & set(player2_positions)
    seen = set()
    common_ordered_unique = [pos for pos in player1_positions if pos in player2_positions and pos != "None" and not (pos in seen or seen.add(pos))]
    ...
    if position_number==1:
        PLAYER_POSITION=positions_name_dict.get(_safe_pos(position_players1), "Defensa")
        print(PLAYER_POSITION)
        #print(f"PLAYER POSITION: {PLAYER_POSITION}")
        player_position_text=positions_name_dict1.get(_safe_pos(general_stats_player1["general_position"]), "Defensa")
    elif position_number==2 and position_players2!="No second position":
        PLAYER_POSITION=positions_name_dict.get(_safe_pos(position_players2), "Defensa")
        print(PLAYER_POSITION)
        player_position_text=positions_name_dict1.get(_safe_pos(general_stats_player1["general_position2"]), "Defensa")
    elif position_number==3 and position_players3!="No third position":
        PLAYER_POSITION=positions_name_dict.get(_safe_pos(position_players3), "Defensa")
        print(PLAYER_POSITION)
        player_position_text=positions_name_dict1.get(_safe_pos(general_stats_player1["general_position3"]), "Defensa")
    else:
    ...
    player_foot1=stats_player1["Pie"] if "Pie" in stats_player1.index else None
    if pd.isna(player_foot1) or str(player_foot1).strip() in ["", "nan", "None", "Unknown"]:
        player_foot2 = "Desconocido"
    ...
    player_foot2 = stats_player2["Pie"] if "Pie" in stats_player2.index else None
    if pd.isna(player_foot2) or str(player_foot2).strip() in ["", "nan", "None", "Unknown"]:
        player_foot2 = "Desconocido"
    
    player_allpositions2 = stats_player2["Posición específica"]
    player_birth_country2 = stats_player2["País de nacimiento"]
    
    folder_skillcorner = Path(BASE_DIR) / "report_gen_opta" / "SKILLCORNER"
    ...
    if (player_minutes1>min_minutes) and (player_minutes2>min_minutes):
        result = pizzaplot_player(wyscout_file1,wyscout_file2,parameters_file,player_id1,player_id2,min_minutes,position_number,color_selection_og,league=current_league,season=season)
        plt.close("all")  # liberar memoria de figuras matplotlib
        if result is None or any(x is None for x in result):
            print(f"[ERROR] No se pudo generar el pizza plot. Las jugadoras pueden no compartir posición o no tener suficientes datos.")
            return None
        fig1,output_path1,fig2,output_path2,fig3,output_path3 = result
    else:
        print(f"Alguna jugadora ha jugado menos de {min_minutes} minutos, selecciona otras jugadoras o baja el número de minutos necesarios.")
        return None
    
    of_score1,of_letter1,def_score1,def_letter1,full_score1,full_letter1,number_of_players_position1,Rank_of1,Rank_def1,Rank_full1=notas_tacticas(PLAYER_POSITION,wyscout_file1,parameters_file,player_id1,min_minutes,pesos_favorito=pesos_favorito)
    of_score2,of_letter2,def_score2,def_letter2,full_score2,full_letter2,number_of_players_position2,Rank_of2,Rank_def2,Rank_full2=notas_tacticas(PLAYER_POSITION,wyscout_file2,parameters_file,player_id2,min_minutes,pesos_favorito=pesos_favorito)
    # print(of_score)
    # print(def_score)
    
    try:
        player1_score, player1_letter, number_of_players_position_fis1, playerranking1 = notas_fisicas(
            PLAYER_POSITION,
            wyscout_file1,
            player_name1,
            parameters_file,
        ...
        print(f"No hay notas físicas para {player_name1}: {e}")
        player1_score, player1_letter, number_of_players_position_fis1, playerranking1 = 0, "-", 0, 0

    try:
        player_score2, player_letter2, number_of_players_position_fis2, playerranking2 = notas_fisicas(
            PLAYER_POSITION,
            wyscout_file2,
            player_name2,
            parameters_file,
        ...
        print(f"No hay notas físicas para {player_name2}: {e}")
        player_score2, player_letter2, number_of_players_position_fis2, playerranking2 = 0, "-", 0, 0

    try:
        phys_result = pizzaplot_phys(
            player_name1, player_name2, PLAYER_POSITION, folder_skillcorner, league=current_league, color="#FFFFFF"
        )
        if phys_result is None or phys_result[0] is None:
            raise ValueError("pizzaplot_phys devolvió None")
        fig4, output_phys, player1_params, player2_params, parameters1_show, phys_average1, real_j1, real_j2 = phys_result
        has_physical_data = True
        print("Datos físicos cargados correctamente.")
    except Exception as e:
        print(f"No hay datos físicos para generar las secciones físicas: {e}")
        has_physical_data = False
        output_phys = None
        player1_params, player2_params, parameters1_show, phys_average1, real_j1, real_j2 = [], [], [], [], [], []
    finally:
        plt.close("all")  # liberar memoria de figuras matplotlib
    ...
    PIZZAPLOT_PHYS=output_phys if has_physical_data else None
   ...
   # TEAM_LOGO = complete_teampath
    ...
    paths_graficas_2D=graficas_2D(wyscout_file1,wyscout_file2,player_id1,player_id2,PLAYER_POSITION,min_minutes,folder_skillcorner,color_selection_og)
    ...
    parrafo2=Paragraph(f"Ficha de la jugadora",sub_heading)
    ...
    data1 = [
        ["Jugadora","Posiciones", "Edad", "Partidos jugados","Minutos jugados"],
        [player_name1,player_allpositions1, f"{player_age1} años", player_games1,player_minutes1],
        [player_name2,player_allpositions2, f"{player_age2} años", player_games2,player_minutes2]
        ]

    data2 = [
        ["Jugadora", "Pie dominante","País de nacimiento"],
        [player_name1, player_foot1, player_birth_country1],
        [player_name2,player_foot2, player_birth_country2]
        ]

    # Crear tablas
    table1 = Table(data1, colWidths=[100,80, 50, 100, 100])
    table2 = Table(data2, colWidths=[100, 80, 130])
    ...
    d32=of_letter1
    d33=def_letter1
    d34=player1_letter if has_physical_data else "N/D"
    ...
    d42= Rank_of1
    d43=Rank_def1
    d44=playerranking1 if has_physical_data else "N/D"
    
    d312=full_letter2
    d322=of_letter2
    d332=def_letter2
    d342=player_letter2 if has_physical_data else "N/D"
    ...
    if d342=="A":
        full_colord342="#007000"
    elif d342=="B":
        full_colord342="#45B05B" #verde mas clarito
    elif d342=="C":
        full_colord342="#FFBF00"
    else:
        full_colord342="#D2222D"
    ...
    Paragraph_d42=Paragraph(f'<font color="{full_colord342}">{d342} </font>',estilo_notes)
    ...
    d422= Rank_of2
    d432=Rank_def2
    d442=playerranking2 if has_physical_data else "N/D"

    parrafo3=Paragraph(f"Valoraciones",sub_heading)
    front_page_story.append(Spacer(0,3))
    front_page_story.append(parrafo3)
    data_t3=[
        ["Jugadora","Valoración táctica global","Valoración ofensiva","Valoración defensiva", "Valoración física"],
        [player_name1,Paragraph_d1,Paragraph_d2,Paragraph_d3,Paragraph_d4],
        [" ",f"{d41}/{number_of_players_position1}",f"{d42}/{number_of_players_position1}",f"{d43}/{number_of_players_position1}",
         f"{d44}/{number_of_players_position_fis1}" if has_physical_data else "N/D"],
        [player_name2,Paragraph_d12,Paragraph_d22,Paragraph_d32,Paragraph_d42],
        [" ",f"{d412}/{number_of_players_position2}",f"{d422}/{number_of_players_position2}",f"{d432}/{number_of_players_position2}",
         f"{d442}/{number_of_players_position_fis2}" if has_physical_data else "N/D"]
        ]
    style = TableStyle([
    ...
    if has_physical_data:
        ...
        main_story_phys.append(Spacer(1, 15))
    ...
    if has_physical_data:
        ...
        main_story7.append(Paragraph(f"{player_name1} - {player_name2}", cabecera))
        main_story7.append(Spacer(0, 10))
        
        bars_phys = []

        for i in range(len(player1_params)):
            phys_value1 = player1_params[i]
            phys_value2 = player2_params[i]
            text = parameters1_show[i].replace('\n', ' ') 
        ...
        avg_phys = sum(player1_params) / len(player1_params)
        avg_phys2 = sum(player2_params) / len(player2_params)
        ...
        if avg_phys2 >= 75: 
            f_let2, f_col2 = "A", "#007000"
        elif avg_phys2 >= 50: 
            f_let2, f_col2 = "B", "#45B05B"
        elif avg_phys2 >= 25: 
            f_let2, f_col2 = "C", "#FFBF00"
        else: 
            f_let2, f_col2 = "D", "#D2222D"
        ...
        for letter in letters:
            if letter == f_let:
                formatted_phys1 += f'<font color="{f_col}" size="48"><b>{letter}</b></font>&nbsp;  '
            else:
                formatted_phys1 += f"{letter} &nbsp; "
         
        formatted_phys2 = ''
        for letter in letters:
            if letter == f_let2:
                formatted_phys2 += f'<font color="{f_col2}" size="48"><b>{letter}</b></font>&nbsp;  '
            else:
                formatted_phys2 += f"{letter} &nbsp; "
                
        formatted_phys = f"{formatted_phys1} &nbsp | &nbsp {formatted_phys2}"
    ...
    player_arrayof11,average_arrayof1,player_name,player_pos,params_arrayof1=extract_arrays_wyscout2(wyscout_file1,parameters_file,player_id1,"param_of1",min_minutes)
    player_arrayof21,average_arrayof2,player_name,player_pos,params_arrayof2=extract_arrays_wyscout2(wyscout_file1,parameters_file,player_id1,"param_of2",min_minutes)
    
    player_arrayof12,average_arrayof1,player_name,player_pos,params_arrayof1=extract_arrays_wyscout2(wyscout_file2,parameters_file,player_id2,"param_of1",min_minutes)
    player_arrayof22,average_arrayof2,player_name,player_pos,params_arrayof2=extract_arrays_wyscout2(wyscout_file2,parameters_file,player_id2,"param_of2",min_minutes)
    ...
    dict_english={"delantera":"forward","ofmid":"ofensive midfielder","extremo":"winger","defmid":"defensive midfielder","lateral":"fullback","defensa":"center back","portera":"goalkeeper"}
    
    chat_comment = f"Write a paragraph comparing the offensive strengths and weaknesses of Player A and Player B with a SWOT style, both playing as {dict_english[player_pos]}. The parameters being evaluated are: {params_arrayof}.The values for each player are: {player_name1} {player_arrayof1}, {player_name2}: {player_arrayof2}. The average values for the position are: {average_arrayof}.Highlight where each player excels or underperforms compared to each other and to the average, and suggest areas where they could improve. Choose the best player."
    ...
    player_arraydef1,average_arraydef,player_name1,player_pos,params_arraydef=extract_arrays_wyscout2(wyscout_file1,parameters_file,player_id1,"param_def",min_minutes)
    player_arraydef2,average_arraydef,player_name2,player_pos,params_arraydef=extract_arrays_wyscout2(wyscout_file2,parameters_file,player_id2,"param_def",min_minutes)
    
    chat_comment = f"Write a paragraph comparing the defensive strengths and weaknesses of Player A and Player B with a SWOT style, both playing as {dict_english[player_pos]}. The parameters being evaluated are: {params_arraydef}.The values for each player are: {player_name1} {player_arraydef1}, {player_name2}: {player_arraydef2}. The average values for the position are: {average_arraydef}.Highlight where each player excels or underperforms compared to each other and to the average, and suggest areas where they could improve. Choose the best player."
    chat(chat_comment, context, action="translate", language="Spanish",chat_log=None)
    ...
    if has_physical_data:
        chat_comment = f"Write a paragraph comparing the physical strengths and weaknesses of Player A and Player B with a SWOT style, both playing as {dict_english[player_pos]}. The parameters we are evaluating are; {parameters1_show}. The values for each player are: {player_name1} {real_j1}, {player_name2}: {real_j2}. The average values for the position are: {phys_average1}.Highlight where each player excels or underperforms compared to each other and to the average, and suggest areas where they could improve. Choose the best player."
        chat(chat_comment, context, action="translate", language="Spanish",chat_log=None)
    ...
    if PLAYER_POSITION=="Defensa" or PLAYER_POSITION=="Lateral" or PLAYER_POSITION=="Portera":
        ...
        entries = [("1. Estadísticas Ofensivas", "3"),
            ("2. Estadísticas Defensivas", "7"),
            ("3. Estadísticas Físicas", "10"),
            ("4. Anexos", "11")]
    ...
    if has_physical_data and output_phys:
        files_to_delete.append(output_phys)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wyscout_file1", type=str, required=True)
    parser.add_argument("--wyscout_file2", type=str, required=False, default=None)
    ...
    parser.add_argument("--league1", type=str, default="Liga F")
    parser.add_argument("--league2", type=str, default=None)
    ...
    pesos_favorito = None
    if args.pesos_file and _os.path.exists(args.pesos_file):
        with open(args.pesos_file, "r", encoding="utf-8") as _f:
            pesos_favorito = _json.load(_f)

    pesos_fisicos = None
    if args.pesos_file_fisico and _os.path.exists(args.pesos_file_fisico):
        with open(args.pesos_file_fisico, "r", encoding="utf-8") as _f:
            pesos_fisicos = _json.load(_f)

    create_report(
        player_id1=args.player_id1,
        player_id2=args.player_id2,
        wyscout_file1=args.wyscout_file1,
        wyscout_file2=args.wyscout_file2 if args.wyscout_file2 else args.wyscout_file1,
        parameters_file=args.parameters_file,
        position_number=args.position_number,
        min_minutes=args.min_minutes,
        color_selection=args.color_selection,
        summary=args.summary,
        league1=args.league1,
        league2=args.league2 if args.league2 else args.league1,
