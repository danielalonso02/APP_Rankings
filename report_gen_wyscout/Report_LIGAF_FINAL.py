...
def create_report(player_id_analizing,wyscout_file,parameters_file,position_number,min_minutes,color_selection="#FFFFFF",summary=0,current_league=current_l,season="2024/25",pesos_favorito=None):
    ...
    player_foot = stats_player["Pie"] if "Pie" in stats_player.index else None
    if player_foot is None or pd.isna(player_foot) or str(player_foot).strip() in ["", "nan", "None", "Unknown"]:
   ...
   # TEAM_LOGO = complete_teampath
    ...
    data1 = [
        ...
        [player_allpositions, f"{PLAYER_AGE} años", player_games,player_minutes]
        ]

    data2 = [
        ["Pie dominante", "Pasaporte","País de nacimiento"],
        [player_foot, player_nationality,player_birth_country]
        ]

    # Crear tablas
    table1 = Table(data1, colWidths=[100, 60, 130, 100])
    table2 = Table(data2, colWidths=[130, 130, 130])
    ...
    chat_comment = f"Write a paragraph highlighting the ofensive strengths and weaknesses of the player {player_name} in the position {dict_english[player_pos]} with a SWOT style. The parameters we are evaluating are; {params_arrayof}. The player's values are: {player_arrayof}. The average values are: {average_arrayof}"
    ...
    chat_comment = f"Write a paragraph highlighting the defensive strengths and weaknesses of the player {player_name} in the position {dict_english[player_pos]} with a SWOT style. The parameters we are evaluating are; {params_arraydef}. The player's values are: {player_arraydef}. The average values are: {average_arraydef}"
    chat(chat_comment, context, action="translate", language="Spanish",chat_log=None)
